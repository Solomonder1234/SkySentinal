...
export default {
    name: 'clown',
    description: 'Toggle clown reaction for a user (Prefix Only).',
    category: 'Troll',
    ...
    run: async (client, interaction) => {
        ...
        const profile = await client.database.prisma.userProfile.upsert({
            ...
            create: { id: targetUser.id, isClowned: true },
            update: { isClowned: true }
        });
        ...
        const embed = EmbedUtils.success('Clown Mode Activated', `🤡 **${targetUser.username}** is now a certified clown.`);
