...
export default {
    name: 'nerd',
    description: 'Toggle nerd reaction for a user (Prefix Only).',
    category: 'Troll',
    ...
    run: async (client, interaction) => {
        ...
        const profile = await client.database.prisma.userProfile.upsert({
            ...
            create: { id: targetUser.id, isNerded: true },
            update: { isNerded: true }
        });
        ...
        const embed = EmbedUtils.success('Nerd Mode Activated', `🤓 **${targetUser.username}** is essentially a nerd emoji now.`);
