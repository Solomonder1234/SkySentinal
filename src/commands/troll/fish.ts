...
export default {
    name: 'fish',
    description: 'Toggle fish reaction for a user (Prefix Only).',
    category: 'Troll',
    ...
    run: async (client, interaction) => {
        ...
        const profile = await client.database.prisma.userProfile.upsert({
            ...
            create: { id: targetUser.id, isFished: true },
            update: { isFished: true }
        });
        ...
        const embed = EmbedUtils.success('Fish Mode Activated', `🐟 **${targetUser.username}** will now get a fish reaction on every message.`);
