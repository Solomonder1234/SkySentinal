    ...
    run: async (client, interaction) => {
        ...
        try {
            if (interaction.guild?.members.me?.roles.highest.position! <= role.position) {
                const errorEmbed = EmbedUtils.error('Failed', 'I cannot remove this role. It is higher than or equal to my highest role.');
                if (interaction instanceof Message) return interaction.reply({ embeds: [errorEmbed] });
                return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            await member.roles.remove(role, reason);
            const successEmbed = EmbedUtils.success('Role Removed', `Removed **${role.name}** from **${user.tag}**.\nReason: ${reason}`);
            ...
            client.logger.error(`Failed to remove role from ${user.tag}:`, error);
            const errorEmbed = EmbedUtils.error('Error', 'An error occurred while removing the role. Check my permissions hierarchy.');
            if (interaction instanceof Message) {
